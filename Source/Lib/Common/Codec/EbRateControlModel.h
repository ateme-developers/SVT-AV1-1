...
#define INTRA_DEVIATION_BRACKET_NUMBER 10
#define INTER_DEVIATION_BRACKET_NUMBER 8
...
/*
 * @struct Holds a min/max value for complexity storing prupose
 */
...
/*
 * @struct Stores a frame suze in bytes for each QP
 * for a specific complexity interval
 * The interval is bounded between scope_start and scope_end
 * For each QP, min is the value at scope_start and max
 * the value at scope_end.
 */
...
/*
 * @struct Stores deviation from model for a specific
 * complexity interval
 */
typedef struct  EbRateControlComplexityModelDeviation_s
 {
    /*
     * @variable uint32_t. Interval lower bound.
     */
     uint32_t    scope_start;

    /*
     * @variable uint32_t. Interval upper bound.
     */
     uint32_t    scope_end;

    /*
     * @variable uint32_t. Deviation observed in the interval.
     */
     uint64_t    deviation;

    /*
     * @variable uint32_t. Number of deviation reported so far.
     */
...
typedef struct  EbRateControlGopInfo_s {
    ...
    /*
     * @variable uint32_t. Intra deviation applied.
     */
    uint32_t    intra_deviation;

    /*
     * @variable uint32_t. Inter deviation applied.
     */
    uint32_t    inter_deviation;

    /*
     * @variable uint32_t. Position in the hierarchy.
     */
    uint8_t     temporal_layer_index;

    /*
     * @variable uint32_t. Number of frame in GOP.
     */
    int64_t     frames_in_sw;
