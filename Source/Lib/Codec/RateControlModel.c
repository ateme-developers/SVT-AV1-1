...
EbErrorType rate_control_model_init(EbRateControlModel *model_ptr, SequenceControlSet_t *sequenceControlSetPtr) {
    ...
    model_ptr->rate_control_mode = sequenceControlSetPtr->static_config.rate_control_mode;

    if (model_ptr->rate_control_mode == RATE_CONTROL_MODE_CBR) {
        rate_control_cbr_init(model_ptr);
    }
...
static void record_new_gop(EbRateControlModel *model_ptr, PictureParentControlSet_t *picture_ptr) {
    ...
    EbRateControlGopInfo *gop_ptr = &model_ptr->gop_infos[pictureNumber];

    EbBlockOnMutex(model_ptr->model_mutex);

    gop_ptr->index = pictureNumber;
    gop_ptr->exists = EB_TRUE;

    uint32_t complexity = estimate_gop_complexity(model_ptr, gop_ptr);
    EbRateControlComplexityModelDeviation *deviation_model = EB_NULL;
    ...
    gop_ptr->complexity = complexity;
    gop_ptr->model_variation = deviation_model->deviation;

    if (model_ptr->rate_control_mode == RATE_CONTROL_MODE_ABR) {
        rate_control_record_new_gop_abr(model_ptr, gop_ptr);
    } else if (model_ptr->rate_control_mode == RATE_CONTROL_MODE_CBR) {
        rate_control_record_new_gop_cbr(model_ptr, gop_ptr);
    }
    ...
    if (pictureNumber != 0) {
        ...
        previousGop->length = gop_ptr->index - previousGop->index;
    }
