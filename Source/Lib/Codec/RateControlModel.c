...
static uint8_t DELTA_LEVELS[7] = {4, 8, 10, 12, 13, 14, 15};
...
EbErrorType rate_control_model_init(EbRateControlModel *model_ptr, SequenceControlSet_t *sequence_control_set_ptr) {
    uint32_t number_of_frame = sequence_control_set_ptr->static_config.framesToBeEncoded;
    ...
    model_ptr->levels = sequence_control_set_ptr->static_config.hierarchical_levels;
...
EbErrorType rate_control_update_model(EbRateControlModel *model_ptr, PictureParentControlSet_t *picture_ptr) {
    ...
    printf("\n{\"type\": \"rateControlFrameCompleted\", \"frameType\": \"%s\", \"pictureNumber\": \"%d\", \"size\": \"%d\", \"qp\": \"%d\", \"extra\": \"%ld\", \"actua\
lSize\": \"%ld\", \"desiredSize\": \"%ld\", \"deviation\": \"%f\"}\n",
        ...
        delta_bytes,
        size,
        (picture_ptr->av1FrameType == INTER_FRAME) ? frame->desired_size : gop->expected_intra_size,
        (picture_ptr->av1FrameType == INTER_FRAME) ? (float)frame->desired_size / (float)size : (float)gop->expected_intra_size / (float)size);
    ...
    if (gop->reported_frames == gop->length) {
        ...
        printf("\n{\"type\": \"rateControlGopCompleted\", \"gopNumber\": \"%ld\", \"intraVariation\": \"%f\", \"interVariation\": \"%f\", \"intraComplexity\": \"%ld\",\
 \"interComplexity\": \"%ld\"}\n",
...
uint8_t rate_control_get_quantizer(EbRateControlModel *model_ptr, PictureParentControlSet_t *picture_ptr) {
    ...
    if (type == INTER_FRAME) {
        uint32_t inter_qp = CLIP3(0, MAX_QP_VALUE, base_qp + DELTA_LEVELS[picture_ptr->temporal_layer_index]);
    ...
    if (model_ptr->reported_frames < MAX_AMOUNT_OF_FRAME_FOR_ON_THE_FLY_QP && gop->reported_frames > AMOUNT_OF_REPORTED_FRAMES_TO_TRIGGER_ON_THE_FLY_QP) {
        ...
        if (model_ptr->reported_frames < 300) {
            //delta_inter *= 2;
        }

        printf("DEVIATION: %f. Apply delta %d 10 - %ld\n", deviation, delta_inter, abs((float)deviation * 10));
...
static void record_new_gop(EbRateControlModel *model_ptr, PictureParentControlSet_t *picture_ptr) {
    ...
    if (model_ptr->reported_frames < 150) {
        divider = 2;
    } else if (model_ptr->reported_frames < 300) {
        divider = 2;
    } else {
        divider = 2;
    }

    int64_t extra = (double)delta_bytes / (double)divider;

    // printf("\n\n EXTRA: %ld\n", extra);
    // printf("Compelxity %ld/%ld (%f/%f)\n", gop->complexity, estimate_gop_complexity(model_ptr, gop), model_deviation->deviation, model_inter_deviation->deviation);
    ...
    for (unsigned int qp = 0; qp <= MAX_QP_VALUE; qp++) {
        ...
        if (model_inter->scope_end == MAX_COMPLEXITY) {
            pitch = PITCH_ON_MAX_COMPLEXITY_FOR_INTER_FRAMES;
        }
        ...
        if (size > projected_gop_size || qp == MAX_QP_VALUE) {
            // printf("Decide QP %ld. Intra = %ld. Inter = %d\n", qp, complexity_size_intra, complexity_size_inter);
            gop->qp = qp;
    ...
    printf("\n{\"type\": \"rateControlGopCreated\", \"gopNumber\": \"%ld\", \"qp\": \"%ld\", \"expectedSize\": \"%ld\", \"expectedIntraSize\": \"%ld\", \"espectedInter\
Size\": \"%ld\"}\n",
        gop->index / model_ptr->intra_period,
        gop->qp,
...
uint64_t compute_inter_size(EbRateControlModel *model_ptr, EbRateControlGopInfo *gop_ptr, EbRateControlComplexityModel *model_inter, uint32_t complexity_inter, uint64_t base_qp) {
           ...
           position <= gop_ptr->picture_ptr->frames_in_sw) 
    {
        ...
        uint32_t inter_qp = CLIP3(0, MAX_QP_VALUE, base_qp + DELTA_LEVELS[current->picture_ptr->temporal_layer_index]);
        ...
        current->desired_size = inter_size;
