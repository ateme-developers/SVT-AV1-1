...
uint32_t get_inter_qp_for_size(EbRateControlModel *model_ptr, uint32_t desired_size, uint32_t complexity) {
    ...
    if (!complexity) {
        ...
        for (unsigned int i = 0; i < sizeof(DEFAULT_COMPLEXITY_MODEL); i++) {
        ...
        for (unsigned int i = 5; i < MAX_QP_VALUE; i++) {
            ...
            uint32_t complexity_steps = (complexity % complexity_model->max_size[i]) - complexity_model->scope_start;
...
static void record_new_gop(EbRateControlModel *model_ptr, PictureParentControlSet_t *picture_ptr) {
    ...
    if (extra > 0 && (uint64_t)extra > (gop->desired_size / 2)) {
      extra = gop->desired_size / 2;
    } else if (extra < 0 && (uint64_t)-extra > (gop->desired_size / 2)) {
      extra = -(gop->desired_size / 2);
    }
